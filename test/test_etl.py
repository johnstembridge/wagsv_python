...
from back_end.interface import get_member_by_name, get_booking, save_booking, get_player_by_name, get_event, save_event_result, get_player, save_handicaps
from models.wags_db import Guest, Handicap
...
class TestEtl(unittest.TestCase):
    ...
    def _test_etl_handicaps(self):
        hcap_file = os.path.join(TestData.data_location, 'hcaps.tab')

        def lu_hcap(rec, key, year):
            return rec[key].startswith(year)

        all_hcaps = Table(*get_records(hcap_file, 'date', '2018', lu_hcap))
        all_hcaps.coerce_columns(['player', 'status'], int)
        all_hcaps.coerce_column('handicap', Decimal)
        all_hcaps.coerce_column('status', PlayerStatus)
        all_hcaps.coerce_column_using('date', parse_date)
        all_hcaps.rename_column('player', 'player_id')
        save_handicaps(all_hcaps)
